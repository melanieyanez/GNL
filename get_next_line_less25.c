...
/*   Updated: 2022/12/09 16:28:59 by myanez-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	get_line_no_nl(fd, &len, &buffer, &stash);
	if (check_stash(buffer, '\n') != -1)
	{
		get_line_with_nl(&line, buffer, &stash, result);
		return (line);
	}
	if (ft_strlen(stash) > 0)
	{
		get_last_line(&line, &stash, result);
		return (line);
	}
	return (NULL);
}

void	get_line_no_nl(int fd, int *len, char **buffer, char **stash)
{
...
void	get_line_with_nl(char **line, char *buffer, char **stash, char **result)
{
	add_to_stash(buffer, stash);
...
void	get_last_line(char **line, char **stash, char **result)
{
